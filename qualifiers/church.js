...
export function churchTurn(r) {
    ...
    if (offensive == false){
        ...
        if (!((dangerCrusader || dangerPreacher) && r.me.turn <= 50 && preacherCounter < 2) && 
            ((danger || prophetCounter < r.me.turn / r.map.length * 4) && r.karbonite > SPECS.UNITS[SPECS.PROPHET].CONSTRUCTION_KARBONITE && r.fuel > SPECS.UNITS[SPECS.PROPHET].CONSTRUCTION_FUEL + 2)) {
            if (r.me.turn < 10 || danger || (r.karbonite > SPECS.UNITS[SPECS.PROPHET].CONSTRUCTION_KARBONITE + 50 && r.fuel > SPECS.UNITS[SPECS.PROPHET].CONSTRUCTION_FUEL + 200)){
                var buildDirection = findBuildDirection(r, r.me.x, r.me.y)
                if (buildDirection != null) {
